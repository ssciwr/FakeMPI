/*
   This is a special set of bindings for uni-processor use of MPI by the PETSc
   library.
...
#define MPI_STATUSES_IGNORE (MPI_Status*)0
  ...
  /* 32-bit packing scheme: [combiner:4 | type-index:8 | count:12 |
   * base-bytes:8] */
...
#define MPI_LONG_LONG (4 << 20 | 1 << 8 | (int)sizeof(long long))
...
#define MPI_UNSIGNED_LONG_LONG                                                 \
  (5 << 20 | 1 << 8 | (int)sizeof(unsigned long long))
...
#define MPI_sizeof_default(datatype)                                           \
  ((((datatype) >> 8) & 0xfff) * ((datatype)&0xff))
...
#define MPI_Buffer_attach(buffer, size)                                        \
  (MPIUNI_ARG(buffer), MPIUNI_ARG(size), MPI_SUCCESS)
...
#define MPI_Wait(request, status)                                              \
  (MPIUNI_ARG(request), MPIUNI_ARG(status), MPI_SUCCESS)
#define MPI_Test(request, flag, status)                                        \
  (MPIUNI_ARG(request), MPIUNI_ARG(status), *(flag) = 0, MPI_SUCCESS)
...
#define MPI_Test_cancelled(status, flag)                                       \
  (MPIUNI_ARG(status), *(flag) = 0, MPI_SUCCESS)
...
#define MPI_Startall(count, array_of_requests)                                 \
  (MPIUNI_ARG(count), MPIUNI_ARG(array_of_requests), MPI_SUCCESS)
...
#define MPI_Type_contiguous(count, oldtype, newtype)                           \
  (*(newtype) = (MPI_COMBINER_CONTIGUOUS << 28) | ((oldtype)&0x0ff00000) |     \
                (((oldtype) >> 8 & 0xfff) * (count)) << 8 | ((oldtype)&0xff),  \
   MPI_SUCCESS)
...
#define MPI_Type_size(datatype, size)                                          \
  (*(size) = MPI_sizeof((datatype)), MPI_SUCCESS)
...
#define MPI_Op_create(function, commute, op)                                   \
  (MPIUNI_ARG(function), MPIUNI_ARG(commute), MPIUNI_ARG(op), MPI_SUCCESS)
#define MPI_Op_free(op) (*(op) = MPI_OP_NULL, MPI_SUCCESS)

#define MPI_Group_size(group, size)                                            \
  (MPIUNI_ARG(group), *(size) = 1, MPI_SUCCESS)
#define MPI_Group_rank(group, rank)                                            \
  (MPIUNI_ARG(group), *(rank) = 0, MPI_SUCCESS)
...
#define MPI_Group_compare(group1, group2, result)                              \
  (MPIUNI_ARG(group1), MPIUNI_ARG(group2), *(result) = 1, MPI_SUCCESS)
#define MPI_Group_union(group1, group2, newgroup)                              \
  (MPIUNI_ARG(group1), MPIUNI_ARG(group2), *(newgroup) = 1, MPI_SUCCESS)
...
#define MPI_Comm_compare(comm1, comm2, result)                                 \
  (MPIUNI_ARG(comm1), MPIUNI_ARG(comm2), *(result) = MPI_IDENT, MPI_SUCCESS)
#define MPI_Comm_split(comm, color, key, newcomm)                              \
  (MPIUNI_ARG(color), MPIUNI_ARG(key), MPI_Comm_dup(comm, newcomm))
...
#define MPI_Get_processor_name(name, result_len)                               \
  (*(result_len) = 9,                                                          \
   MPIUNI_Memcpy(name, "localhost", 10 * MPI_sizeof(MPI_CHAR)))
#define MPI_Errhandler_create(function, errhandler)                            \
  (MPIUNI_ARG(function), *(errhandler) = MPI_ERRORS_RETURN, MPI_SUCCESS)
#define MPI_Errhandler_set(comm, errhandler)                                   \
  (MPIUNI_ARG(comm), MPIUNI_ARG(errhandler), MPI_SUCCESS)
#define MPI_Errhandler_get(comm, errhandler)                                   \
  (MPIUNI_ARG(comm), (*errhandler) = MPI_ERRORS_RETURN, MPI_SUCCESS)
#define MPI_Errhandler_free(errhandler)                                        \
  (*(errhandler) = MPI_ERRHANDLER_NULL, MPI_SUCCESS)
#define MPI_Error_string(errorcode, string, result_len)                        \
  (MPIUNI_ARG(errorcode),                                                      \
   (errorcode == MPI_ERR_NOSUPPORT)                                            \
     ? (*(result_len) = 35,                                                    \
        MPIUNI_Memcpy(string,                                                  \
                      "MPI error, not supported by MPI-uni",                   \
                      35 * MPI_sizeof(MPI_CHAR)))                              \
     : (*(result_len) = 9,                                                     \
        MPIUNI_Memcpy(string, "MPI error", 9 * MPI_sizeof(MPI_CHAR))))
...
#define MPI_File_close(mpi_fh)                                                 \
  (MPIUNI_ARG(mpi_fh), MPIUni_Abort(MPI_COMM_WORLD, 0))
...
#define MPI_Register_datarep(                                                  \
  name, read_conv_fn, write_conv_fn, extent_fn, state)                         \
  (MPIUNI_ARG(name),                                                           \
